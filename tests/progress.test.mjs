...
function fixture(){
 ...
 const fill={style:{vars:new Map(),setProperty(k,v){this.vars.set(k,v);}},classList:{toggle(name,on){if(on)classes.add(name);else classes.delete(name);}}};
...
test('resume preflight stays visible across zero events without inventing a percentage',()=>{
 ...
 assert.equal(f.classes.has('indeterminate'),true);assert.notEqual(f.fill.style.vars.get('--progress'),'0');
...
test('known progress replaces unknown activity and never retreats between backend phases',()=>{
 ...
 assert.equal(f.fill.style.vars.get('--progress'),'0.06');assert.equal(f.attrs.get('aria-valuenow'),'6');
...
test('a new operation resets the prior completed state and bounds malformed values',()=>{
 ...
 assert.equal(f.fill.style.vars.get('--progress'),'1');f.progress.setBusy(true);
 ...
 assert.equal(f.fill.style.vars.get('--progress'),'1');assert.equal(f.classes.size,0);
});
