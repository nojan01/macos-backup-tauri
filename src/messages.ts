...
export const messagePairs: [string,string][] = [
  ...
  ['AppleArchive mit LZFSE: Jedes neue Archiv wird vollständig zurückgelesen. Für die temporäre Kopie wird zusätzlicher freier Speicher geprüft.','AppleArchive with LZFSE: every new archive is fully read back. Additional free space for the temporary copy is checked.'],
  ...
  ['Speicherplatzprüfung: {0} GB frei, ~{1} GB neu/geändert (benötigt ≥ {2} GB mit Rücklesekopie und Reserve)','Free space check: {0} GB free, ~{1} GB new/changed (need ≥ {2} GB with readback and reserve)'],
