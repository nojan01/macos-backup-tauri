...
export const messagePairs: [string,string][] = [
  ...
  ['Teilarchiv {0} im RAM komprimieren und zurücklesen','Compressing and reading back archive part {0} in memory'],
  ['Teilarchiv {0} im RAM geprüft; Prüfkopie freigegeben','Archive part {0} verified in memory; verification copy released'],
  ['Wenig freier RAM: Teilarchive werden auf der internen SSD verarbeitet','Low available memory: processing archive parts on the internal SSD'],
  ['Genug freier RAM: weitere Teilarchive im Arbeitsspeicher','Enough free memory: processing subsequent archive parts in RAM'],
  ['Teilarchiv {0} geprüft; temporäre Prüfkopie entfernt','Archive part {0} verified; temporary verification copy removed'],
  ['Teilarchiv {0}/{1} zurückgelesen und geprüft','Archive part {0}/{1} read back and verified'],
  ...
  ['AppleArchive mit LZFSE: Bei genügend freiem RAM werden 128-MiB-Teile vollständig im Arbeitsspeicher erstellt und zurückgelesen. Sonst werden Teile bis 1 GiB über den temporären Arbeitsbereich der internen SSD geprüft (5 GiB frei erforderlich).','AppleArchive with LZFSE: with sufficient free memory, 128 MiB parts are created and read back entirely in RAM. Otherwise, parts up to 1 GiB are checked using temporary space on the internal SSD (5 GiB free required).'],
  ['AppleArchive-Struktur prüfen','Validating AppleArchive structure'],
  ['AppleArchive mit LZFSE erstellen: {0}','Creating AppleArchive with LZFSE: {0}'],
  ['AppleArchive entpacken und macOS-Metadaten wiederherstellen','Extracting AppleArchive and restoring macOS metadata'],
  ...
  ['Speicherplatzprüfung: {0} GiB frei, ~{1} GiB neu/geändert (Spitzenbedarf inklusive Teilprüfung und Reserve: {2} GiB)','Free space check: {0} GiB free, ~{1} GiB new/changed (peak including readback and reserve: {2} GiB)'],
  ['Nicht genug Speicherplatz auf dem Ziel: {0} GiB frei, Spitzenbedarf ~{1} GiB (neu/geändert {2} GiB, einschließlich Teilprüfung und Reserve).','Insufficient target space: {0} GiB free, peak requirement ~{1} GiB (new/changed {2} GiB, including readback and reserve).'],
  ['Speicherplanung: Große Quellen zuerst; auch große Einzeldateien werden aufgeteilt. Der Rücklese-Arbeitsbereich bleibt unabhängig von der Quellgröße begrenzt.','Space planning: large sources first; large individual files are split too. Readback workspace is bounded independently of source size.'],
