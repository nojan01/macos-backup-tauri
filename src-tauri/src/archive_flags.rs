...
fn name(root: &str) -> &'static str {
    if root.eq_ignore_ascii_case(NAME) || root.eq_ignore_ascii_case(&format!("._{NAME}")) { ALTERNATE } else { NAME }
}
pub(super) fn write(stage: &Path, flags: &Flags) -> Result<PathBuf, String> {
    ...
    if bytes.len() as u64 > MAX_BYTES { return Err("Dateiflag-Metadaten sind zu groß".into()); }
    ...
    header.set_size(bytes.len() as u64);header.set_mode(0o600);
    header.set_uid(0);header.set_gid(0);header.set_mtime(0);
    header.set_username(OWNER).map_err(|e|e.to_string())?;header.set_cksum();
    tar.append_data(&mut header, name, bytes.as_slice()).map_err(|e|e.to_string())?;
...
pub(super) fn read<R: Read>(path: &Path, entry: &mut tar::Entry<'_, R>) -> Result<Option<Flags>, String> {
    if !(path == Path::new(NAME) || path == Path::new(ALTERNATE)) || !entry.header().entry_type().is_file() {
        return Ok(None);
    }
    if entry.header().username().map_err(|e|e.to_string())? != Some(OWNER) { return Ok(None); }
    let mut prefix = Vec::new();
    entry.by_ref().take(MAGIC.len() as u64).read_to_end(&mut prefix).map_err(|e|e.to_string())?;
    if prefix != MAGIC { return Err("Ungültige Signatur der Dateiflag-Metadaten".into()); }
    let mut data = Vec::new();
    entry.by_ref().take(MAX_BYTES + 1).read_to_end(&mut data).map_err(|e|e.to_string())?;
    if data.len() as u64 > MAX_BYTES { return Err("Dateiflag-Metadaten sind zu groß".into()); }
    let flags: Flags = serde_json::from_slice(&data).map_err(|e|format!("Ungültige Dateiflag-Metadaten: {e}"))?;
    if path != Path::new(name(&flags.root)) { return Err("Dateiflag-Metadaten haben einen ungültigen Namen".into()); }
    Ok(Some(flags))
}
pub(super) fn validate(flags: &Flags, entries: &BTreeMap<PathBuf, tar::EntryType>) -> Result<(), String> {
    super::restore::validate_component(&flags.root)?;
    super::restore::require_root(&entries.keys().cloned().collect(), std::ffi::OsStr::new(&flags.root))?;
    let mut seen = BTreeSet::new();
    for entry in &flags.entries {
        let rel = Path::new(&entry.path);
        if !rel.components().all(|c| matches!(c,Component::Normal(_))) || entry.path.contains('\0') {
            return Err(format!("Unsicherer Pfad in Dateiflag-Metadaten: {:?}",entry.path));
        }
        let path = Path::new(&flags.root).join(rel);
        if !entries.contains_key(&path) || !seen.insert(path) || entry.flags == 0 {
            return Err(format!("Ungültiger oder doppelter Dateiflag-Eintrag: {:?}",entry.path));
        }
    }
    Ok(())
}
...
pub(super) fn set(path: &Path, expected: u32) -> Result<(), String> {
    let before = fs::symlink_metadata(path).map_err(|e|e.to_string())?.st_flags();
    if before == expected { return Ok(()); }
    ...
    let writable = libc::UF_NODUMP | libc::UF_IMMUTABLE | libc::UF_APPEND |
        libc::UF_OPAQUE | libc::UF_TRACKED | libc::UF_HIDDEN;
    ...
    unsafe extern "C" { fn lchflags(path: *const libc::c_char, flags: libc::c_uint) -> libc::c_int; }
    if unsafe { lchflags(cpath.as_ptr(),expected) } != 0 {
        return Err(format!("{}: Dateiflags 0x{expected:08x} wiederherstellen: {}",path.display(),std::io::Error::last_os_error()));
    }
    let actual = fs::symlink_metadata(path).map_err(|e|e.to_string())?.st_flags();
    if actual != expected { return Err(format!("{}: Dateiflags stimmen nach Wiederherstellung nicht überein (erwartet 0x{expected:08x}, vorhanden 0x{actual:08x})",path.display())); }
    Ok(())
}
pub(super) fn apply(target: &Path, flags: &Flags) -> Result<(), String> {
    ...
    for record in records {
        ...
        let relative = if record.path.is_empty() {PathBuf::from(&flags.root)} else {Path::new(&flags.root).join(&record.path)};
        for parent in relative.ancestors().skip(1).filter(|p|!p.as_os_str().is_empty()) {
            ...
            if !md.is_dir() || md.file_type().is_symlink() { return Err("Dateiflags dürfen nicht durch symbolische Links gesetzt werden".into()); }
        }
