...
pub(crate) fn create_archive(mut command: impl FnMut() -> std::process::Command) -> Result<std::process::Output, String> {
    let mut last = None;
    let result = retry(|| {
        ...
        last = Some(output);
        if denied { Err(io::Error::from_raw_os_error(libc::EPERM)) } else { Ok(()) }
    });
    match result {
...
fn retry_with<T>(
    ...
    loop {
        if cancelled() { return Err(AccessError::Cancelled); }
        match operation() {
...
#[cfg(test)]
pub(crate) fn enable_real_lock_state() { REAL_LOCK_STATE.with(|value| value.set(true)); }
fn screen_locked() -> Option<bool> {
    ...
    #[cfg(test)]
    if !REAL_LOCK_STATE.with(|value| value.get()) { return Some(false); }
    read_screen_locked()
}
fn read_screen_locked() -> Option<bool> {
    ...
    unsafe extern "C" {
        fn CFStringCreateWithCString(allocator: *const c_void, text: *const std::ffi::c_char, encoding: u32) -> *const c_void;
    ...
    unsafe {
        ...
        let key = CFStringCreateWithCString(std::ptr::null(), c"IOConsoleLocked".as_ptr(), 0x08000100);
        if key.is_null() { IOObjectRelease(root); return None; }
        let value = IORegistryEntryCreateCFProperty(root, key, std::ptr::null(), 0);
        CFRelease(key);
        IOObjectRelease(root);
        if value.is_null() { return None; }
        let result = if CFGetTypeID(value) == CFBooleanGetTypeID() { Some(CFBooleanGetValue(value) != 0) } else { None };
        CFRelease(value);
        result
    }
}
...
mod tests {
    use super::*;
    use std::cell::Cell;
    fn denied() -> io::Error { io::Error::from_raw_os_error(libc::EPERM) }

    #[test]
    fn denied_access_waits_for_unlock_then_retries_without_skipping() {
        let ticks=Cell::new(0);let attempts=Cell::new(0);let mut states=Vec::new();
        let value=retry_with(|| {
            attempts.set(attempts.get()+1);
            if ticks.get()<3 {Err(denied())} else {Ok(b"actual data")}
        },||Some(ticks.get()<3),||false,||ticks.set(ticks.get()+1),|state|states.push(state)).unwrap();
        assert_eq!(value,b"actual data");assert_eq!(attempts.get(),2);assert_eq!(ticks.get(),3);assert_eq!(states,vec![true,false]);
    }

    #[test]
    fn permission_denial_after_unlock_is_an_error() {
        let ticks=Cell::new(0);let attempts=Cell::new(0);
        let result:Result<(),_>=retry_with(||{attempts.set(attempts.get()+1);Err(denied())},||Some(ticks.get()==0),||false,||ticks.set(1),|_|{});
        assert!(matches!(result,Err(AccessError::Io(_))));assert_eq!(attempts.get(),2);
    }

    #[test]
    fn cancellation_while_locked_stops_without_retrying_the_read() {
        let ticks=Cell::new(0);let attempts=Cell::new(0);
        let result:Result<(),_>=retry_with(||{attempts.set(attempts.get()+1);Err(denied())},||Some(true),||ticks.get()==2,||ticks.set(ticks.get()+1),|_|{});
        assert!(matches!(result,Err(AccessError::Cancelled)));assert_eq!(attempts.get(),1);
    }

    #[test]
    fn unlocked_unknown_and_io_errors_do_not_wait() {
        for state in [None,Some(false)] {
            let result:Result<(),_>=retry_with(||Err(denied()),||state,||false,||panic!("must not wait"),|_|panic!("must not pause"));
            assert!(matches!(result,Err(AccessError::Io(_))));
        }
        let result:Result<(),_>=retry_with(||Err(io::Error::from_raw_os_error(libc::EIO)),||Some(true),||false,||panic!("must not wait"),|_|panic!("must not pause"));
        assert!(matches!(result,Err(AccessError::Io(_))));
    }

    #[test]
    fn readable_files_continue_while_locked() {
        assert_eq!(retry_with(||Ok(42),||Some(true),||false,||panic!("must not wait"),|_|{}).unwrap(),42);
    }
    #[test]
    #[ignore = "manual read-only screen-lock probe"]
    fn actual_screen_lock_state() { println!("SCREEN_LOCKED={:?}", read_screen_locked()); }
    ...
    fn failed_archive_creation_keeps_exit_status_and_stderr() {
        ...
        let output = create_archive(|| {
            ...
            command.args(["-c", "printf 'fixture: Operation not permitted' >&2; exit 7"]);
            command
        }).unwrap();
