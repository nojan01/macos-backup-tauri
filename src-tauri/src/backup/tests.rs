...
fn empty_and_corrupt_metadata_checkpoints_remain_resumable() {
    ...
    assert_eq!(
        list_resumable_backups(d.0.to_str().unwrap().into(), "standard".into())
            .unwrap()
            .len(),
        1
    );
...
fn large_file_scan_reports_progress_inside_file_and_matches_manifest() {
    ...
    let payload = vec![0x5a; 5 * 1024 * 1024 + 17];
    fs::write(&path, &payload).unwrap();
    let mut events = Vec::new();
    let snapshot = scan_with_activity(&path, &mut |a| events.push(a.clone())).unwrap();
    assert!(events
        .iter()
        .any(|a| a.bytes > 0 && a.bytes < (payload.len() as u64)));
...
fn progress_does_not_prevent_cancelling_inside_large_file() {
    ...
    let result = scan_with_activity(&path, &mut |a| {
        if a.bytes > 0 {
            BACKUP_CANCELLED.store(true, Ordering::SeqCst);
        }
    });
...
fn git_runtime_socket_does_not_block_archiving_or_restore() {
    ...
    let snapshot = scan_with_activity(&source, &mut |a| {
        if a.skipped_socket {
            skipped.insert(a.current_file.clone());
        }
    })
    .unwrap();
...
fn explicit_socket_source_and_nested_fifo_remain_errors() {
    let _guard = OperationGuard::acquire().unwrap();
    let d = PrivateDir::new(Path::new("/tmp"), "socket-test").unwrap();
    let socket = d.0.join("endpoint");
    let _listener = std::os::unix::net::UnixListener::bind(&socket).unwrap();
    assert!(compute_snapshot(&socket)
        .unwrap_err()
        .contains("Laufzeit-Socket"));
...
fn archive_validation_and_restore_merge_honor_cancellation() {
    ...
    let target = d.0.join("target");
    assert!(merge_tree(&source, &target, true)
        .unwrap_err()
...
fn preflight_reports_all_missing_roots_before_deep_scan() {
    let d = fixture();
    let existing = d.0.join("Documents");
    ...
    let fifo = existing.join("pipe");
    let name = CString::new(fifo.as_os_str().as_bytes()).unwrap();
    assert_eq!(unsafe { libc::mkfifo(name.as_ptr(), 0o600) }, 0);
    let selected = vec![
        existing.to_str().unwrap().into(),
    ...
    assert!(error.contains("Code/User"));
    assert!(error.contains("another-missing"));
    assert!(!error.contains("FIFO"));
    assert!(error.contains("noch keine Dateiinhalte"));
    assert!(!d.0.join("destination").exists());
}
#[test]
fn preflight_accepts_existing_roots_and_dangling_links() {
    let d = fixture();
    let dir = d.0.join("Documents");
    fs::create_dir(&dir).unwrap();
    let file = d.0.join("config");
...
fn preflight_rejects_unreadable_roots_overlap_and_duplicates() {
    ...
    let dir = d.0.join("Documents");
    fs::create_dir(&dir).unwrap();
    let selected = vec!["~/Documents".into()];
    assert!(
        validate_selected_sources(&selected, &dir.join("backup"), &d.0)
            .unwrap_err()
            .contains("ineinander")
    );
    let duplicate = vec!["~/Documents".into(), dir.to_str().unwrap().into()];
    assert!(
        validate_selected_sources(&duplicate, &d.0.join("dest"), &d.0)
            .unwrap_err()
            .contains("mehrfach")
    );
    fs::set_permissions(&dir, fs::Permissions::from_mode(0)).unwrap();
    let result = validate_selected_sources(&selected, &d.0.join("dest"), &d.0);
    fs::set_permissions(&dir, fs::Permissions::from_mode(0o700)).unwrap();
    assert!(result.unwrap_err().contains("nicht lesbar"));
}
...
fn representative_backup_roundtrip() {
    ...
    println!(
        "REPRESENTATIVE_BACKUP {:.3}s {} entries",
...
fn regenerated_provenance_is_allowed_only_in_readback_not_source_guards() {
    ...
    for entry in &mut expected {
        entry.xattrs.insert(
            "com.apple.provenance".into(),
            "different source application".into(),
        );
    }
    verify_archive_source(&archive, "Documents", &expected).unwrap();
    assert!(ensure_unchanged(&source, &expected).is_err());
    expected[0].xattrs.insert(
        "com.example.required".into(),
        "missing or corrupt metadata".into(),
    );
    let error = verify_archive_source(&archive, "Documents", &expected).unwrap_err();
    assert!(error.contains("bei Documents:"));
    assert!(error.contains("com.example.required"));
}
#[test]
fn provenance_exception_never_hides_content_acl_resource_forks_or_other_xattrs() {
    ...
    for field in [
        ...
        changed
            .xattrs
            .insert("com.apple.provenance".into(), "regenerated".into());
        match field {
            "hash" => changed.hash = "bad".into(),
            "acl" => changed.acl = "different".into(),
            _ => {
                changed.xattrs.insert(field.into(), "bad".into());
            }
        }
        assert!(
            !readback_differences(&changed, &original).is_empty(),
            "{field} mismatch hidden"
        );
    }
}
...
fn actual_directory_root_metadata_roundtrip() {
    ...
    let md = fs::symlink_metadata(&root).unwrap();
    assert!(md.is_dir());
    let mut attrs = BTreeMap::new();
    for key in xattr::list(&root).unwrap() {
        attrs.insert(
            key.to_str().unwrap().to_string(),
            format!(
                "{:x}",
                Sha256::digest(xattr::get(&root, &key).unwrap().unwrap())
            ),
        );
    }
...
fn actual_source_backup_finalize_and_test_restore() {
    if std::env::var("BACKUP_PROBE_WAIT_FOR_UNLOCK").as_deref() == Ok("1") {
        crate::protected_access::enable_real_lock_state();
    }
    let source =
        PathBuf::from(std::env::var("BACKUP_PROBE_SOURCE").expect("Explicit source required"));
    let d = fixture();
    let backup = d.0.join("backup");
    fs::create_dir(&backup).unwrap();
    let expected = compute_snapshot(&source).unwrap();
    ...
    let restored =
        compute_snapshot(&Path::new(&result.extracted_path).join(source.file_name().unwrap()))
            .unwrap();
    assert_eq!(restored.len(), expected.len());
    for (actual, original) in restored.iter().zip(&expected) {
        assert!(readback_differences(actual, original).is_empty());
    }
    println!(
        "ACTUAL_SOURCE_BACKUP_AND_RESTORE passed: {} files, {} bytes",
        result.file_count, result.bytes_extracted
    );
}
...
fn access_preflight_collects_nested_denials_across_sources() {
    let d = fixture();
    let one = d.0.join("first");
    let two = d.0.join("second");
    fs::create_dir_all(one.join("nested")).unwrap();
    fs::create_dir(&two).unwrap();
    let blocked = [one.join("nested/unreadable"), two.join("another")];
    for p in &blocked {
        fs::write(p, b"private").unwrap();
        fs::set_permissions(p, fs::Permissions::from_mode(0)).unwrap();
    }
    let result = validate_source_access(
        &[one.to_str().unwrap().into(), two.to_str().unwrap().into()],
        &d.0,
    );
    for p in &blocked {
    ...
    for p in &blocked {
        assert!(error.contains(p.to_str().unwrap()), "{error}");
    }
...
fn source_replacement_during_read_restarts_hash_from_zero() {
    ...
    fs::write(&source, vec![b'a'; 2 * 1024 * 1024]).unwrap();
    fs::write(&replacement, b"current-version").unwrap();
    let mut replaced = false;
    let snapshot = scan_with_activity(&source, &mut |a| {
        if a.bytes > 0 && !replaced {
    ...
    .unwrap();
    assert!(replaced);
    assert_eq!(snapshot, compute_snapshot(&source).unwrap());
    assert_eq!(
        snapshot[0].hash,
        format!("{:x}", Sha256::digest(b"current-version"))
    );
...
fn continuous_replacement_exhausts_bounded_retries() {
    ...
    let error = scan_with_activity(&source, &mut |a| {
        if a.bytes > 0 && !a.boundary {
            let p = d.0.join("replacement");
            fs::write(&p, b"updated").unwrap();
            fs::rename(&p, &source).unwrap();
            replacements += 1;
        }
    })
    .unwrap_err();
    assert_eq!(replacements, 3);
    assert!(error.contains("3 Versuche"));
}

#[test]
fn entry_retries_never_retry_permission_errors_and_honor_cancel() {
    let d = fixture();
    let path = d.0.join("denied");
    let mut attempts = 0;
    let result: Result<(), String> = retry_entry(&path, || {
        attempts += 1;
        Err(EntryReadError::Other(access_error(
            &path,
            "Dateiinhalt lesen",
    ...
    assert!(result.unwrap_err().contains("Dateiinhalt lesen"));
    attempts = 0;
    let result: Result<(), String> = retry_entry(&path, || {
        attempts += 1;
        BACKUP_CANCELLED.store(true, Ordering::SeqCst);
        Err(EntryReadError::Changed)
    });
    BACKUP_CANCELLED.store(false, Ordering::SeqCst);
    assert_eq!(attempts, 1);
    assert!(result.unwrap_err().contains("abgebrochen"));
...
fn actual_selected_sources_access_preflight() {
    let config: BackupConfig =
        serde_json::from_slice(&fs::read(std::env::var("BACKUP_PROBE_CONFIG").unwrap()).unwrap())
            .unwrap();
...
fn quarantine_survives_roundtrip_with_os_regenerated_value() {
    ...
    xattr::set(
        &source,
        "com.apple.quarantine",
        b"0086;6a9f2328;com.apple.cfprefsd;",
    )
    .unwrap();
    let expected = compute_snapshot(&source).unwrap();
    ...
    let mut missing = actual[0].clone();
    missing.xattrs.remove("com.apple.quarantine");
    assert!(readback_differences(&missing, &expected[0])
        .contains(&"Erweitertes Attribut com.apple.quarantine".into()));
    let mut source_changed = expected[0].clone();
    source_changed
        .xattrs
        .insert("com.apple.quarantine".into(), "changed".into());
...
fn final_source_check_identifies_changed_added_and_removed_paths() {
    ...
    let source = d.0.join("source");
    fs::create_dir(&source).unwrap();
    fs::write(source.join("changed"), b"old").unwrap();
    fs::write(source.join("removed"), b"gone").unwrap();
    let expected = compute_snapshot(&source).unwrap();
    fs::write(source.join("changed"), b"new").unwrap();
    ...
    assert!(error.contains("removed: entfernt"));
    assert!(error.contains("added: neu hinzugekommen"));
}

#[test]
fn tracked_document_roundtrips_with_hidden_flag_content_and_metadata() {
    let d = fixture();
    let source = d.0.join("document");
    fs::write(&source, b"document data").unwrap();
    xattr::set(&source, "com.example.backup", b"metadata").unwrap();
    let path = CString::new(source.as_os_str().as_bytes()).unwrap();
    assert_eq!(
        unsafe { libc::chflags(path.as_ptr(), libc::UF_TRACKED | libc::UF_HIDDEN) },
        0
    );
    let expected = compute_snapshot(&source).unwrap();
    ...
    assert_eq!(
        actual[0].xattrs.get("com.example.backup"),
        expected[0].xattrs.get("com.example.backup")
    );
    assert!(readback_differences(&actual[0], &expected[0]).is_empty());
    ensure_unchanged(&source, &expected).unwrap();
    assert_eq!(unsafe { libc::chflags(path.as_ptr(), libc::UF_HIDDEN) }, 0);
    assert!(ensure_unchanged(&source, &expected)
        .unwrap_err()
        .contains("Dateiflags"));
}
...
fn readback_rejects_every_changed_flag_bit() {
    ...
    for bit in 0..32 {
        ...
        actual.flags ^= 1u32 << bit;
        assert!(readback_differences(&actual, &expected)
            .iter()
            .any(|d| d.contains("Dateiflags")));
    }
}
...
fn compressed_file_roundtrip_retains_content_and_compression_flag() {
    ...
    let compressed = d.0.join("compressed");
    fs::write(&plain, vec![b'x'; 128 * 1024]).unwrap();
...
fn tracked_flags_survive_directory_merge_hardlinks_and_symlinks_without_live_source() {
    ...
    for (actual, expected) in restored.iter().zip(&baseline) {
        assert!(
    ...
    assert_eq!(
        fs::metadata(target.join("hardlink")).unwrap().ino(),
        fs::metadata(target.join("nested/document")).unwrap().ino()
    );
...
fn internal_flags_filename_never_replaces_user_data() {
    let d = fixture();
    for name in [
        ".macos-backup-suite-flags-v1",
        ".macos-backup-suite-flags-v1-alternate",
        ".MACOS-BACKUP-SUITE-FLAGS-V1",
    ] {
        let source = d.0.join(name);
        fs::write(
            &source,
            b"macOS Backup Suite file flags v1\n{arbitrary user data}",
        )
        .unwrap();
...
fn missing_compression_state_is_never_faked_by_setting_its_flag() {
    let d = fixture();
    let source = d.0.join("plain");
    fs::write(&source, b"plain data").unwrap();
    assert!(crate::archive_flags::set(&source, libc::UF_COMPRESSED)
        .unwrap_err()
        .contains("Systemflags"));
...
fn immutable_files_and_directories_restore_with_their_protection_flags() {
    let owned = ReadbackDir(fixture());
    let d = &owned.0;
    let source = d.0.join("locked");
    fs::create_dir(&source).unwrap();
    let file = source.join("document");
    fs::write(&file, b"protected data").unwrap();
    ...
    for existing in [false, true] {
        ...
        for (a, b) in restored.iter().zip(&expected) {
            assert!(
                readback_differences(a, b).is_empty(),
                "{:?}",
                readback_differences(a, b)
            );
        }
    }
}
...
fn incremental_reuse_ignores_only_the_snapshot_mount_device_number() {
    ...
    let source = d.0.join("file");
    fs::write(&source, b"data").unwrap();
    let a = compute_snapshot(&source).unwrap();
    let mut b = a.clone();
    b[0].dev += 1;
    assert!(same_source_version(&a, &b));
    assert_ne!(a, b);
    b[0].ino += 1;
    assert!(!same_source_version(&a, &b));
    b[0].ino -= 1;
    b[0].xattrs
        .insert("com.apple.lastuseddate#PS".into(), "changed".into());
    assert!(!same_source_version(&a, &b));
}
...
fn frozen_backup_survives_live_content_and_usage_attribute_changes() {
    ...
    let baseline = compute_snapshot(&stable).unwrap();
...
fn actual_frozen_source_backup_finalize_and_test_restore() {
    ...
    let meta = BackupMetadata {
        profile_id: "standard".into(),
        profile_name: "Standard".into(),
        timestamp: "20260908-140000".into(),
        items: vec![BackupItem {
            path: source.to_str().unwrap().into(),
            archive: name,
            hash: hash_file(&archive).unwrap(),
    ...
    println!(
        "ACTUAL_FROZEN_BACKUP_AND_RESTORE passed: {} files, {} bytes",
        result.file_count, result.bytes_extracted
    );
}
...
fn literal_appledouble_names_and_resource_forks_survive_backup_and_merge() {
    let d = fixture();
    let source = d.0.join("tree");
    fs::create_dir_all(source.join("._directory")).unwrap();
    for (name, bytes) in [
        ("image", b"image data".as_slice()),
        ("._image", b"independent sidecar"),
        ("._orphan", b"no sibling"),
        ("._directory/file", b"nested data"),
    ] {
    ...
    xattr::set(
        source.join("image"),
        "com.apple.ResourceFork",
        b"resource fork must not replace sidecar",
    )
    .unwrap();
    xattr::set(
        source.join("._image"),
        "com.apple.ResourceFork",
        b"sidecar has its own resource fork",
    )
    .unwrap();
    ...
    fs::create_dir_all(&target).unwrap();
    staged_restore(&archive, &target, true).unwrap();
    let actual = compute_snapshot(&target).unwrap();
    ...
    assert_eq!(
        fs::metadata(target.join("hardlink")).unwrap().ino(),
        fs::metadata(target.join("._image")).unwrap().ino()
    );
}
...
fn literal_appledouble_root_and_metadata_companion_are_user_files() {
    ...
    for name in [
        "._document",
        "._.macos-backup-suite-flags-v1",
        "._.macos-backup-suite-flags-v1-alternate",
    ] {
        let source = d.0.join(name);
...
fn missing_readback_entries_report_exact_paths() {
    ...
    assert!(error.contains("._missing"));
    assert!(error.contains("fehlend"));
}
...
fn resume_reuses_only_unchanged_sources_and_verified_archives() {
    ...
    current[0].dev += 1;
    assert!(
        verified_resume_item(&backup, &inventory, "source", name, &current, &items)
            .unwrap()
            .is_some()
    );
    current[0]
        .xattrs
        .insert("com.example.changed".into(), "value".into());
    assert!(
        verified_resume_item(&backup, &inventory, "source", name, &current, &items)
            .unwrap()
            .is_none()
    ...
    set_times(&source, initial[0].m, initial[0].mn);
    assert!(verified_resume_item(
        &backup,
        &inventory,
        "source",
        name,
        &compute_snapshot(&source).unwrap(),
        &items
    )
    .unwrap()
    .is_none());
    ...
    assert!(
        verified_resume_item(&backup, &inventory, "source", name, &initial, &items)
            .unwrap()
            .is_none()
    );
...
fn remaining_frozen_sources_backup_and_restore() {
    let sources: Vec<PathBuf> = serde_json::from_slice(
        &fs::read(std::env::var("BACKUP_PROBE_SOURCES_JSON").unwrap()).unwrap(),
    )
    .unwrap();
    ...
    for source in &sources {
        ...
        items.push(BackupItem {
            ...
            hash: hash_file(&archive).unwrap(),
            archive_size_bytes: fs::metadata(&archive).unwrap().len(),
            source_size_bytes: expected.iter().map(|e| e.s).sum(),
        });
        guards.push((stable, expected));
        println!("PROBE_READBACK_PASSED {}", source.display());
    }
    let meta = BackupMetadata {
        profile_id: "standard".into(),
        profile_name: "Standard".into(),
        timestamp: "20260908-182000".into(),
        total_source_size_bytes: items.iter().map(|e| e.source_size_bytes).sum(),
        items,
        hash_algorithm: "sha256".into(),
        start_time: String::new(),
        end_time: String::new(),
    ...
    for (source, (_, expected)) in sources.iter().zip(&guards) {
        ...
        let actual = compute_snapshot(
            &Path::new(&restored.extracted_path).join(source.file_name().unwrap()),
        )
        .unwrap();
        assert_eq!(actual.len(), expected.len());
        for (a, b) in actual.iter().zip(expected) {
