...
fn full_native_readback(
    ...
    let parent=archive.parent().ok_or("Archiv ohne übergeordnetes Verzeichnis")?;
    ...
    require_free_space(parent,RESERVE.saturating_add(payload).saturating_add(payload/10))?;
    let stage=PrivateDir::new(parent,".readback-full")?;
    let _phase=crate::work_progress::Phase::enter("PAX-Metadatenprobe unvollständig – vollständige Rückleseprüfung auf dem Backup-Laufwerk");
...
pub(super) fn verify_contents_and_metadata(archive: &Path, root: &str, expected: &[ManifestEntry], stage: &Path) -> Result<Vec<ManifestEntry>, String> {
    space_preflight()?;
    ...
    let mut expected_paths: BTreeSet<_> = expected
        .iter()
        .map(|entry| {
            if entry.p.is_empty() { PathBuf::from(root) } else { Path::new(root).join(&entry.p) }
        })
        .collect();
    ...
    let mut input = open_archive(archive)?;
    {
        ...
        for entry in tar.entries().map_err(|e|fail(archive,e))? {
            ...
            if kind.is_file() && !keep_payload {
                ...
                if has_pax_path {output.append(&header,probe.as_slice()).map_err(|e|fail(archive,e))?;}
                else {output.append_data(&mut header,&path,probe.as_slice()).map_err(|e|fail(archive,e))?;}
            } else if let Some(link)=link {
                if kind.is_hard_link() {hardlinks.push((path.clone(),relative_path(&link)?));}
                if has_pax_path {output.append(&header,std::io::empty()).map_err(|e|fail(archive,e))?;}
                else {output.append_link(&mut header,&path,&link).map_err(|e|fail(archive,e))?;}
            } else {
                if has_pax_path {output.append(&header,&mut entry).map_err(|e|fail(archive,e))?;}
                else {output.append_data(&mut header,&path,&mut entry).map_err(|e|fail(archive,e))?;}
            }
        }
    }
    input.finish()?;
    ...
    let scanned=snapshot_with_phase(&probes.join(root),"Rückgelesene Dateiattribute prüfen")?;
    ...
    let mut by_path: BTreeMap<_,_> = scanned.into_iter().map(|entry|(entry.p.clone(),entry)).collect();
    let directory_probe_inexact=expected.iter().filter(|entry|entry.kind=="dir").any(|expected_entry| {
        by_path.get(&expected_entry.p)
    ...
    for expected_entry in expected {
        let mut entry=by_path.remove(&expected_entry.p)
            .ok_or_else(||fail(archive,format!("Metadata probe omitted archive entry: {}",if expected_entry.p.is_empty(){root.to_string()}else{format!("{root}/{}",expected_entry.p)})))?;
        entry.p=expected_entry.p.clone();
        actual.push(entry);
    }
    let extra: Vec<_> = by_path.keys().take(10).collect();
    if !extra.is_empty() {
        return Err(fail(archive,format!("Metadata probe created unexpected entries: {extra:?}")));
    }
...
mod tests {
    ...
    fn binary_multiline_pax_metadata_roundtrips_backup_and_restore() {
        ...
        let source=owned.0 .0.join("pax-source"); fs::create_dir(&source).unwrap();
        let name=format!("{}\nsecond-line", "long-name-".repeat(16));
        let path=source.join(&name); fs::write(&path,b"original payload").unwrap();
        ...
        let destination=owned.0 .0.join("restored"); fs::create_dir(&destination).unwrap();
        ...
        assert_eq!(xattr::get(&restored,"com.example.backup-pax").unwrap().unwrap(),value);
        let actual=compute_snapshot(&destination.join("pax-source")).unwrap();
        for (a,e) in actual.iter().zip(&expected) { assert!(readback_differences(a,e).is_empty()); }
    ...
    fn metadata_probe_keeps_the_exact_nested_pax_entry_set() {
        ...
        fs::write(source.join("GitHub/other-project/src-tauri/Info.plist"),b"plist").unwrap();
        ...
        xattr::set(source.join("GitHub/project/README.md"),"com.example.backup-pax",b"binary\0value\nsecond line").unwrap();
        xattr::set(source.join(&long).join("long-file.txt"),"com.example.backup-pax",b"long PAX path metadata").unwrap();
        let changed=source.join("GitHub/project");
        let name=CString::new(changed.as_os_str().as_bytes()).unwrap();
        let times=[libc::timespec {tv_sec:1_700_000_000,tv_nsec:123_456_789};2];
        assert_eq!(unsafe {libc::utimensat(libc::AT_FDCWD,name.as_ptr(),times.as_ptr(),0)},0);
        let expected=compute_snapshot(&source).unwrap();
        let archive=owned.0 .0.join("documents.tar.gz");
        create_verified_archive_from_snapshot(&source,&archive,true,&expected).unwrap();
        let stage=owned.0 .0.join("readback");fs::create_dir(&stage).unwrap();
        let actual=verify_contents_and_metadata(&archive,"Documents",&expected,&stage).unwrap();
        assert_eq!(
            actual.iter().map(|entry|entry.p.as_str()).collect::<Vec<_>>(),
            expected.iter().map(|entry|entry.p.as_str()).collect::<Vec<_>>(),
        );
        for (actual,expected) in actual.iter().zip(&expected) {
            assert!(readback_differences(actual,expected).is_empty(),"{:?}",readback_differences(actual,expected));
        }
    }
    #[test]
    fn large_payload_is_fully_verified_without_a_full_temporary_copy() {
        ...
        create_verified_archive_from_snapshot(&source,&archive,true,&expected).unwrap();
        let stage=owned.0 .0.join("readback");fs::create_dir(&stage).unwrap();
        let actual=verify_contents_and_metadata(&archive,"virtual-disk.hds",&expected,&stage).unwrap();
